...
to_field 'cho_creator', column('personal_name'), parse_csv, strip, arabic_script_lang_or_default('und-Arab', 'und-Latn')
...
to_field 'cho_language', column('language_languageTerm'), parse_csv, split(';'), strip, normalize_language, lang('en')
to_field 'cho_language', column('language_languageTerm'), parse_csv, split(';'), strip, normalize_language, translation_map('lang_ar_from_en'), lang('ar-Arab')
to_field 'cho_publisher', column('originInfo_publisher'), parse_csv, gsub(']،', ''), arabic_script_lang_or_default('ar-Arab', 'en')
...
to_field 'cho_type', column('genre'), parse_csv, lang('en')
...
to_field 'agg_is_shown_at' do |_record, accumulator, context|
  accumulator << transform_values(
    context,
    'wr_id' => [column('shown_at'), parse_csv, at_index(0), strip],
...
to_field 'agg_preview' do |_record, accumulator, context|
  accumulator << transform_values(
    context,
    'wr_id' => [column('preview'), parse_csv, gsub('?width=150&height=150&usethumb=y', '/full/,150/0/default.jpg'), gsub('/view/', 'iiif'), gsub('full/,150/0', 'full/,400/0'), strip],
    'wr_is_referenced_by' => [column('shown_at'), parse_csv, at_index(0), split('/iiif/'), at_index(-1), split('/full/'), at_index(0), prepend('https://iiif.lib.harvard.edu/manifests/ids:')]
  )
end
