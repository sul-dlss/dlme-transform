...
to_field 'agg_data_provider_collection', literal('auc-maps'), translation_map('agg_collection_from_provider_id'), lang('en')
to_field 'agg_data_provider_collection', literal('auc-maps'), translation_map('agg_collection_from_provider_id'), translation_map('agg_collection_ar_from_en'), lang('ar-Arab')
to_field 'agg_data_provider_collection_id', literal('auc-maps')
...
# Cho Other
to_field 'cho_contributor', extract_json('.contributors'), strip, lang('en')
...
to_field 'cho_language', extract_json('.language'), strip, split('; '), normalize_language, lang('en')
to_field 'cho_language', extract_json('.language'), strip, split('; '), normalize_language, translation_map('lang_ar_from_en'), lang('ar-Arab')
to_field 'cho_publisher', extract_json('.publisher'), strip, lang('en')
to_field 'cho_spatial', extract_json('.coverage-spatial'), strip, lang('en')
to_field 'cho_subject', extract_json('.subject'), strip, lang('en')
to_field 'cho_subject', extract_json('.genre-aat'), strip, lang('en')
...
to_field 'agg_is_shown_at' do |_record, accumulator, context|
  accumulator << transform_values(
    context,
    'wr_dc_rights' => [literal('To inquire about permissions or reproductions, contact'\
                                'the Rare Books and Special Collections Library, The American'\
                                'University in Cairo at +20.2.2615.3676 or rbscl-ref@aucegypt.edu.')],
