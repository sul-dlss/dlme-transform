...
module IntFromString
  ...
  def negative_first_four_digits(date_str)
    Regexp.last_match(1) if date_str =~ /^(-\d{4})/
  end
  ...
  def negative_4digits_after_hyphen(date_str)
    Regexp.last_match(1) if date_str =~ /-\d{4}\s*(?:-|–|–|or|to)\s*(-\d{4})/
  end
  ...
  DATESTR_REGEX = /\b(\d{4})\b.*?\/.*?\b(\d{4})\b/im # rubocop:disable Style/RegexpLiteral
